...
select * from 
(select *,rank() over(partition by category order by quantity_sold desc) as Qty_sold from products) as subquery
where Qty_sold=1;
...
select * from 
(select *,rank() over(partition by category order by quantity_sold asc) as Qty_sold from products) as subquery
where Qty_sold=1;
